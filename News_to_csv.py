...
def extraerTiempo(file):
    ...
    url="headlines/final/periodico=ElTiempo/year="+str(fecha.year)+"/month="+str(fecha.strftime('%m'))+"/day="+str(fecha.strftime('%d'))+"/elTiempo"
...
def extraerEspectador(file):
    ...
    url="headlines/final/periodico=ElEspectador/year="+str(fecha.year)+"/month="+str(fecha.strftime('%m'))+"/day="+str(fecha.strftime('%d'))+"/elEspectador"
    s3_resource.Object(S3_BUCKET, url+'{}.csv'.format(fecha.strftime('%Y-%m-%d'))).put(Body=texto)
